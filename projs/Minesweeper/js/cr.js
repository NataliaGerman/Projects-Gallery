...
//CR: when rendering a board, it should be according to a board array 
function renderBoard() {
  ...
  for (var i = 0; i < gLevel.size; i++) {
    ...
    for (var j = 0; j < gLevel.size; j++) {
      ...
      onmousedown="cellMarked(event,this,${i},${j})"> </td>`
    }
    strHTML += '</tr>'
  }
  strHTML += '</tbody></table>';
...
function renderCell(location, elcell) {
  ...
  if (gBoard[location.i][location.j].isMine === false && 
    gBoard[location.i][location.j].minesAroundCount === 0) var value = EMPTY;
  if (gBoard[location.i][location.j].isMine === false && 
    gBoard[location.i][location.j].minesAroundCount !== 0) var value = gBoard[location.i][location.j].minesAroundCount;
...
function cellMarked(ev, elcell, i, j) {
  if (ev.button === 2) {
    if (gBoard[i][j].isMarked === false && gScore > 0) {
      ...
      var elscore = document.querySelector('.score span')
      elscore.innerText = gScore;
      // CR: it's not a good counting, it'll give wrong results
      if (gBoard[i][j].isMine === true) {
        gGame.markedCount++
      }
    } else return;
