...
function getAllPossibleCoordsBishop(pieceCoord) {
    ...
    for (var idx = pieceCoord.i - 1; idx >= 0 && j >= 0; idx--) {
        var coord = { i: idx, j: j-- };
        if (!isEmptyCell(coord)) break;
        res.push(coord);
    }
...
function getAllPossibleCoordsQueen(cellCoord) {
    var res = [];
    var res1 = getAllPossibleCoordsBishop(cellCoord);
    var res2 = getAllPossibleCoordsRook(cellCoord);
...
function getAllPossibleCoordsKnight(pieceCoord) {
    ...
    for (var i = pieceCoord.i - 2; i <= pieceCoord.i + 2; i++) {
        ...
        for (var j = pieceCoord.j - 2; j <= pieceCoord.j + 2; j++) {
            if (j < 0 || j >= 8) continue;
            if (j === pieceCoord.j) continue;
            if (Math.abs(i - pieceCoord.i) === Math.abs(j - pieceCoord.j)) continue;
            var coord = { i: i, j: j };
            if (!isEmptyCell(coord)) continue;
            //console.log (coord)
            res.push(coord);
